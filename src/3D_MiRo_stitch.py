...
class DisplayStitched(object):
    ...
    def stitchImages(self):
        ...
        # Prevents the program crashing if an unexpected failure occurs
        except: 
        ...
        if status == 0:
            try:
                ...
                if finalH > 550 and finalW > 1530:
                    ...
                    # 3D Glasses Implementation ---------------------

                    # Converts the stitch to one channel, so it can be reconstructed into 
                    # red / cyan
                    grey = cv2.cvtColor(finalStitch, cv2.COLOR_BGR2GRAY)

                    img_width, img_length, _ = np.shape(finalStitch)
                    
                    # Used for the empty channels (Blue and Green for Red, Red for Cyan)
                    empty = np.zeros([img_width,img_length],dtype=np.uint8)

                    # Because each colour is shifted slightly, the image needs to be made slightly
                    # bigger and an empty section added to the start / end depending on which way
                    # it shifts
                    red_cyan_overlap = 10
                    empty_bit = np.zeros([img_width,red_cyan_overlap,3],dtype=np.uint8)

                    # Creates the red
                    red = cv2.merge([empty,empty,grey])
                    red = red[0:img_width, red_cyan_overlap:img_length]
                    red = cv2.hconcat([red, empty_bit])

                    # Creates the cyan 
                    cyan = cv2.merge([grey,grey,empty])
                    cyan = cyan[0:img_width, 0:img_length-red_cyan_overlap]
                    cyan = cv2.hconcat([empty_bit, cyan])

                    # Overlays the two versions, using equal weighting
                    finalStitch = cv2.addWeighted(red,0.5,cyan,0.5,0)
                    #finalStitch = cv2.resize(finalStitch,None, fx = 2.5, fy=2.5)
